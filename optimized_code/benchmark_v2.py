...
def save_csv(records):
    path = os.path.join(OUTPUT_DIR, 'benchmark_v2_results.csv')
...
def plot_speedup(records):
    """Speedup of v1 and v2 over baseline."""
    ...
    fig.tight_layout()
    out = os.path.join(OUTPUT_DIR, 'benchmark_v2_speedup.png')
...
def plot_absolute(records):
    """Absolute wall-clock time for all three versions."""
    fig, axes = plt.subplots(1, 2, figsize=(13, 5), sharey=True)
    for ax, steps in zip(axes, STEPS_VALUES):
        rows = _sub(records, steps)
        caps = [r['particleCap'] for r in rows]
    ...
    fig.tight_layout()
    out = os.path.join(OUTPUT_DIR, 'benchmark_v2_absolute.png')
    fig.savefig(out, dpi=150, bbox_inches='tight')
    print(f'Saved {out}')
    plt.close(fig)
...
def plot_breakdown(records):
    ...
    labels   = ['Baseline', 'v1 (aggr opt)', 'v2 (+vec radial)']
    c_aggr   = ['#1f77b4', '#ff7f0e', '#2ca02c']
    c_rad    = ['#aec7e8', '#ffbb78', '#98df8a']
    ...
    fig.tight_layout()
    out = os.path.join(OUTPUT_DIR, 'benchmark_v2_breakdown.png')
    fig.savefig(out, dpi=150, bbox_inches='tight')
    print(f'Saved {out}')
    plt.close(fig)
